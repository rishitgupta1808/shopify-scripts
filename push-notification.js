...
<div id="mySidenav" class="sidenav" style="display: flex;">
    <h4 class="side1" style="margin-top: 32.5px;display: none;margin-left: 20px;">{{%BELL_STATEMENT%}}</h4>
    <a href="javascript:void(0)" onclick="{{%ALERT%}}({{%PRODUCT_ID%}})" class="side2" style="font-weight:'bold';font-size:20px;margin-top: 20px;margin-left:20px;display: none;color: blue;">ALERT</a>
    <a href="javascript:void(0)" class="closebtn" onclick="closeNav()">&times;</a>
</div>


<div style="font-size:20px;cursor:pointer" onclick="openNav()" class="bell">
  <div ${window.screen.width>600?(style="background-color: black;width: 50px;height: 80px;"):(style="background-color: black;width: 40px;height: 60px;")}>
    ...
    await fetch(
      `${s_url}/api/subscription/createSubscription/${window.Shopify.shop}`,
      {
        ...
        body: JSON.stringify({
          ...
          backInStockAndPriceDrop: true,
          productId,
          priceDrop: true,
          backInStock: false,
          customerInfo: JSON.parse(
            localStorage.getItem("superAssistant-ipInfo")
          ),
          browserName,
          devicePlatform,
        }),
...
async function send() {
  //serviceworker scrip inject by allowing in notification
  // if(){
  ...
  const register = await navigator.serviceWorker.register(
    `https://${Shopify.shop}/apps/super-assistant-push/push-notification/serviceWorker.js`,
    {
...
console.log(subscription)
  ...
  console.log("fetch", customerInfo);
